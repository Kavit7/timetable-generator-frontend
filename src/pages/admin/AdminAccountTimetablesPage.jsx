import React from "react";
import { Link, useParams } from "react-router-dom";

const userTimetables = {
  1: [
    {
      title: "Class 10A Weekly Timetable",
      status: "Published",
      updated: "Today",
    },
    {
      title: "Teacher Duty Timetable",
      status: "Draft",
      updated: "Yesterday",
    },
  ],
  2: [
    {
      title: "Senior School Exam Timetable",
      status: "Published",
      updated: "2 days ago",
    },
  ],
};

const AdminAccountTimetablesPage = () => {
  const { accountId } = useParams();
  const timetableList = userTimetables[accountId] ?? userTimetables["1"];

  return (
    <section className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-sky-500">
            Admin workspace
          </p>
          <h2 className="mt-3 text-3xl font-semibold text-slate-900 sm:text-4xl">
            User timetables
          </h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">
            View all timetables created by this user in one place.
          </p>
        </div>

        <Link
          to="/admin/accounts"
          className="inline-flex items-center justify-center rounded-2xl border border-sky-200 bg-white px-6 py-3 text-sm font-semibold text-sky-700 transition hover:bg-sky-50"
        >
          Back to accounts
        </Link>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        {timetableList.map((item) => (
          <div
            key={item.title}
            className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-100/50"
          >
            <p className="text-sm uppercase tracking-[0.25em] text-sky-500">
              {item.status}
            </p>
            <h3 className="mt-3 text-2xl font-semibold text-slate-900">
              {item.title}
            </h3>
            <p className="mt-2 text-sm text-slate-600">
              Last updated {item.updated}
            </p>
          </div>
        ))}
      </div>
    </section>
  );
};

export default AdminAccountTimetablesPage;
